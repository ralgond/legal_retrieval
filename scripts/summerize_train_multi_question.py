...
all_data = []
for idx, (query_id, query) in tqdm(enumerate(zip(train_df['query_id'], train_df['query'].tolist())), total=len(train_df)):
    prompt=f'''你是一位精通德语的法律检索优化专家。请将以下用户的问题，改写成一个精炼的、适合在法律判决书数据库中进行关键词搜索的查询语句。
              ## 要求：
              0、原始问题的前半部分是事实陈述(context)，后半段是问题(question)，问题(question)有一个或多个；你应该具备识别这个模式的能力。
              1、保留context和question的核心法律关键词（如罪名、法律行为、涉案主体）。
              2、去除context和question的语气词，疑问词和口语化表达。
              3、在context和question中，如果可能，补充该行为的法律专业术语（例如将“偷拿”改为“盗窃”或“职务侵占”）。
...
result_df.to_csv("../data/train_rewrite_split_question_001.csv", index=False)
