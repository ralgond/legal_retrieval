...
def build_split(df: pd.DataFrame, split_name: str,
                ...
                hard_neg_rank_feature_index: int = 0,
                hard_neg_ratio:       int = 3,
                hard_neg_rank_reverse: bool = False):
    """
    ...
    for i, (query_id, query, gold_citations) in enumerate(
        ...
        pos_cid_set = set(pos_cids)
        hard_neg_cid_l = []
        
        # ── 随机负采样（仅 train，val 保持完整） ──────────────────────────────────
        if do_neg_sample and neg_pos_ratio >= 0:

            # 难负样本采样
            cid_rank_score_l = []
            for cid, feat in cid_feat_d.items():
                cid_rank_score_l.append((cid, feat[hard_neg_rank_feature_index])) # cid_rank_score_l包括了所有cid

            
            cid_rank_score_l = sorted(cid_rank_score_l, key=lambda x:x[1], reverse=hard_neg_rank_reverse) # 根据某个特别强的特征做排序
            last_rank = None
            for rank, (cid, _) in enumerate(cid_rank_score_l, start=1):
                last_rank = rank
                if cid not in pos_cid_set:
                    print(f"[{query_id}] found hard neg, rank={rank}")
                    hard_neg_cid_l.append(cid)
                    if len(hard_neg_cid_l) >= hard_neg_ratio * len(pos_cid_set):
                        break

            max_neg = len(pos_cids) * (neg_pos_ratio - hard_neg_ratio)
            cid_rank_score_l = cid_rank_score_l[last_rank:]
            neg_cids = [cid for cid,_ in cid_rank_score_l]
            if len(neg_cids) > max_neg:
                neg_cids = list(rng.choice(neg_cids, size=max_neg, replace=False))
        # ─────────────────────────────────────────────────────────────────────

        sampled_cids = pos_cids + hard_neg_cid_l + neg_cids
        # print(sampled_cids)
        # random.shuffle(sampled_cids)
        print(f"  pos={len(pos_cids)}  hard_neg={len(hard_neg_cid_l)} easy_neg={len(neg_cids)}  "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hard_neg_rank_feature_index", type=int, default=0,
        help="用给定的index所执行的feature对cid进行排序")

    parser.add_argument(
        "--hard_neg_rank_reverse", type=bool, default=False,
        help="用给定的index所执行的feature对cid进行排序")

    parser.add_argument(
        "--hard_neg_ratio", type=int, default=3,
        help="训练集每个正样本保留多少难负样本")
    ...
    build_split(train_df, "train",
                ...
                hard_neg_rank_feature_index=args.hard_neg_rank_feature_index,
                hard_neg_ratio=args.hard_neg_ratio,
                hard_neg_rank_reverse=args.hard_neg_rank_reverse)
