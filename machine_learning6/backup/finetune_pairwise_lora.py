...
# ─────────────────────────────────────────────
# 3. Cross-Encoder（pairwise，LoRA 微调）
# ─────────────────────────────────────────────

class CrossEncoderClassifier(nn.Module):
    """
    Pairwise ranking with LoRA：
    - encoder：AutoModel + LoRA adapter（只训练低秩矩阵，~1% 参数）
    - classifier：线性头，随机初始化
    - Loss：-logsigmoid(pos_score - neg_score)
    ...
    def __init__(
        ...
        if lora_target_modules is None:
            module_names = {n.split(".")[-1] for n, _ in base_model.named_modules()}
            if "query" in module_names and "value" in module_names:
                lora_target_modules = ["query", "value"]       # RoBERTa
            elif "q_proj" in module_names and "v_proj" in module_names:
                lora_target_modules = ["q_proj", "v_proj"]     # LLaMA 系列
            elif "query_proj" in module_names:
                lora_target_modules = ["query_proj", "value_proj"]  # DeBERTa
            else:
                lora_target_modules = ["q", "v"]               # fallback
            logger.info(f"Auto-detected LoRA target_modules: {lora_target_modules}")
        ...
        logger.info(f"CrossEncoderClassifier (LoRA): hidden={hidden_size}, "
...
class Trainer:
    ...
    def _save(self, name: str):
        ...
        # 只保存 LoRA adapter 权重（很小，几十MB），不保存完整模型
    ...
    def train(self):
        ...
        # LoRA 下所有可训练参数（adapter + classifier）用同一个 lr
        try:
...
class CitationClassifier:
    ...
    def __init__(self, model_path: str, base_model_name: str = None):
        ...
        # base_model_name 未指定时从 adapter_config.json 里读取
        self.tokenizer = AutoTokenizer.from_pretrained(model_path)

        # 重建模型：先初始化 CrossEncoderClassifier，再加载 LoRA adapter
        adapter_cfg_path = Path(model_path) / "adapter_config.json"
        if adapter_cfg_path.exists():
            import json as _json
            cfg = _json.loads(adapter_cfg_path.read_text())
            bm_name = base_model_name or cfg.get("base_model_name_or_path", model_path)
        else:
            bm_name = base_model_name or model_path

        self.model = CrossEncoderClassifier(bm_name)
        # 加载 LoRA adapter 权重
        self.model.encoder = PeftModel.from_pretrained(
            self.model.encoder.base_model.model
            if hasattr(self.model.encoder, "base_model") else self.model.encoder,
            model_path,
        )
...
def main():
    ...
    trainer = Trainer(
        ...
        train_ds=train_ds,
        eval_ds=eval_ds,
        output_dir="../data/ml6/checkpoints/legal_swiss_roberta_lora",
        num_epochs=3,
        batch_size=8,
        grad_accum_steps=4,
        lr=2e-4,           # LoRA 通常用比全量微调更大的 lr
        warmup_ratio=0.06,
        bf16=True,
        eval_steps=50000000,
        save_steps=100000,
